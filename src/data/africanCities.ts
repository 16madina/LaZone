...
export const searchAllCities = (query: string = ''): Array<{name: string, country: string}> => {
  ...
  // Villes principales à toujours afficher (capitales et grandes villes)
  const mainCities = [
    'Abidjan', 'Dakar', 'Lagos', 'Accra', 'Nairobi', 'Dar es Salaam', 'Addis Ababa', 
    'Cairo', 'Casablanca', 'Tunis', 'Algiers', 'Tripoli', 'Khartoum', 'Bamako', 
    'Ouagadougou', 'Niamey', 'N\'Djamena', 'Yaoundé', 'Libreville', 'Brazzaville', 
    'Kinshasa', 'Luanda', 'Windhoek', 'Gaborone', 'Johannesburg', 'Cape Town', 
    'Durban', 'Maputo', 'Antananarivo', 'Port Louis', 'Bouaké', 'San-Pédro', 'Yamoussoukro'
  ];
  
  AFRICAN_CITIES_DATA.forEach(country => {
    country.cities.forEach(city => {
      const shouldInclude = normalizedQuery === '' 
        ? mainCities.includes(city.name) // Sans requête, ne montrer que les principales
        : city.name.toLowerCase().includes(normalizedQuery); // Avec requête, chercher partout
  ...
  return results
    ...
    .slice(0, normalizedQuery ? 50 : 30); // Plus de résultats si recherche active
};